//звуки
package com.example.horrormod.init;
import com.example.horrormod.HorrorMod;
import com.sun.java.swing.plaf.windows.WindowsRootPaneUI;
import net.minecraft.registry.Registries;
import net.minecraft.registry.Registry;
import net.minecraft.sound.SoundEvent;
import net.minecraft.util.FixedBufferInputStream;
import net.minecraft.util.Identifier;
import java.lang.annotation.Inherited;
//Звуковые события
//Три разных амбиентных (фоновых/окружающих) звука для ебланов
//дыхание, хруст веток, шепот, рычание и т.п.
public class ModSounds {
    public static SoundEvent HORROR_AMBIENT1;
    public static SoundEvent HORROR_AMBIENT2;
    public static SoundEvent HORROR_AMBIENT3;
    //публичный метод
    //Вызывается один раз при инициализации мода
    //Создает и сохраняет все три звуковых события
    public static void register() {
        HORROR_AMBIENT1 = registerSound("horror_ambient1");
        HORROR_AMBIENT2 = registerSound("horror_ambient2");
        HORROR_AMBIENT3 = registerSound("horror_ambient3");
    }
    //Метод регестрации звука
    //Создается идентификатор звука: horrormod:horror_ambient1
    //Звук регистрируется в реєстр звуков Minecraft (Registries.SOUND_EVENT)
    //Возвращается объект SoundEvent, который можно использовать в коде
    private static SoundEvent registerSound(String id) {
        Identifier identifier = new Identifier(HorrorMod.MOD_ID, id);
        return Registry.register(Registries.SOUND_EVENT, identifier, SoundEvent.of(identifier));
    }
}
